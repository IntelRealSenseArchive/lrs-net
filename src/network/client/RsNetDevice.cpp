...
bool rs_net_device::init_device_data(rs2::software_device sw_device)
{
    ...
    for(int sensor_id = 0; sensor_id < NUM_OF_SENSORS; sensor_id++)
    {
        ...
        for (auto& control : controls)
        {
            ...
            fuflog << "Init sensor " << control.sensorId << ", option '" << control.option << "', value " << control.range.def << "\n";
            fuflog.close();
            ...
            catch (const std::exception& e)
            {
                LOG_ERROR(e.what());
                std::ofstream fuflog("c:\\temp\\rsnet.log", std::ios::out | std::ios::app | std::ios::binary);
                fuflog << "Error setting value for sensor " << control.sensorId << ", option '" << control.option << "', value " << get_option_value(control.sensorId, control.option) << "\n";
                fuflog.close();
            }
        }
    }
            ...
            for(int i = 0; i < NUM_OF_SENSORS; i++)
            {
                ...
                for(rs2_option opt : sw_sensor->get_supported_options())
                {
                    ...
                    if(opt_val != (float)sw_sensor->get_option(opt))
                    {
                        ...
                        std::ofstream fuflog("c:\\temp\\rsnet.log", std::ios::out | std::ios::app | std::ios::binary);
                        fuflog << "Option '" << opt << "' for sensor " << i << " has changed to: " << remote_sensors[i]->sensors_option[opt] << "\n";
                        fuflog.close();
